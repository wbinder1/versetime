...
class SystemManager:
    ...
    def get_known_networks(self):
        """Get known WiFi networks from wpa_supplicant.conf"""
        networks = []
        try:
                ...
                for block in network_blocks:
                    ...
                    for line in lines:
                        ...
                        if line.startswith('ssid='):
                            ...
                            network['password'] = line.split('=')[1].strip().strip('"')
                        elif line.startswith('key_mgmt='):
                            network['encrypted'] = line.split('=')[1].strip() != 'NONE'
                    
                    if 'ssid' in network:
                        networks.append(network)
        except Exception as e:
            print(f"Error reading known networks: {e}")
        
        return networks
    
    def update_network(self, index, ssid, password):
        """Update a known WiFi network"""
        try:
            networks = self.get_known_networks()
            if 0 <= index < len(networks):
                # Update the network at the specified index
                networks[index]['ssid'] = ssid
                networks[index]['password'] = password
                networks[index]['encrypted'] = bool(password and password.strip())
                
                # Rewrite the entire wpa_supplicant.conf file
                self._write_wpa_supplicant_config(networks)
                return True
            return False
        except Exception as e:
    ...
    def delete_network(self, ssid, index):
        """Delete a known WiFi network"""
        try:
            networks = self.get_known_networks()
            if 0 <= index < len(networks) and networks[index]['ssid'] == ssid:
    ...
    def _write_wpa_supplicant_config(self, networks):
        """Write networks to wpa_supplicant.conf"""
        try:
            # Create backup
            backup_file = self.wifi_config_file + '.backup'
            if os.path.exists(self.wifi_config_file):
                import shutil
            ...
            with open(self.wifi_config_file, 'w') as f:
                f.write("# WiFi configuration file\n")
                f.write("# Generated by VerseTime web interface\n\n")
                ...
                for network in networks:
                    ...
                    if network.get('encrypted', False) and network.get('password'):
                        f.write(f'    psk="{network["password"]}"\n')
            ...
            if os.path.exists(backup_file):
                import shutil
                shutil.copy2(backup_file, self.wifi_config_file)
            return False
    ...
    def add_wifi_network(self, ssid, password):
        ...
        try:
            ...
            new_network = {
                'ssid': ssid,
                'password': password,
                'encrypted': bool(password and password.strip())
            }
            networks.append(new_network)
            
            # Write updated configuration
            return self._write_wpa_supplicant_config(networks)
        except Exception as e:
...
def add_wifi():
    ...
    try:
        ...
        if system_manager.add_wifi_network(ssid, password):
            network_type = "protected" if password else "open"
            return jsonify({'success': True, 'message': f'{network_type.capitalize()} WiFi network "{ssid}" added successfully'})
        else:
...
@app.route('/api/known_networks')
def get_known_networks():
    """Get known WiFi networks"""
    networks = system_manager.get_known_networks()
    return jsonify(networks)

@app.route('/api/update_network', methods=['POST'])
def update_network():
    """Update a known WiFi network"""
    try:
        data = request.get_json()
        ...
        if system_manager.update_network(int(index), ssid, password):
            return jsonify({'success': True, 'message': 'Network updated successfully'})
        else:
            return jsonify({'success': False, 'message': 'Failed to update network'})
    except Exception as e:
        return jsonify({'success': False, 'message': f'Error: {str(e)}'})
...
def delete_network():
    ...
    try:
        ...
        if system_manager.delete_network(ssid, int(index)):
            return jsonify({'success': True, 'message': 'Network deleted successfully'})
        else:
            return jsonify({'success': False, 'message': 'Failed to delete network'})
    except Exception as e:
        return jsonify({'success': False, 'message': f'Error: {str(e)}'})
